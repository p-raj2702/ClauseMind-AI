...
const ClauseCard: React.FC<ClauseCardProps> = ({
  ...
  const splitIntoSentences = (text: string | undefined): string[] => {
    if (!text || typeof text !== "string") return [];
    return text
      .split(/(?<=[.?!])\s+(?=<mark>|[A-Z])/) // Handles highlights or capital letters
      .map((s) => s.trim())
      .filter((s) => s.length > 0);
  };
  ...
  return (
    <div className="bg-white/80 backdrop-blur-md border border-blue-100 rounded-2xl shadow-2xl p-6 space-y-5 transition duration-300 hover:shadow-blue-300">
      ...
      <div className="text-green-700 text-sm font-medium">
        ✅ Justification:{" "}
        <span className="text-gray-800">{justification}</span>
      </div>
